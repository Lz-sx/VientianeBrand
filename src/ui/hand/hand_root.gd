...
var start_position = Vector2(DisplayServer.window_get_size().x/2.0, 800)
...
func move():
	var center_num:int = 0
	if card_size%2 != 0:
		center_num = (card_size-1)/2
		for i in card_size:
			var card = get_child(i)
			var angle = (i - center_num) * 2
			var offset = Vector2(sin(deg_to_rad(angle+90)) * angle * 25, \
			cos(deg_to_rad(angle+90)) * angle * 15)
